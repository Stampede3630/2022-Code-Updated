...
public class RobotContainer implements Logged {
  ...
  final SwerveRequest.RobotCentric requestRoboCentric = new SwerveRequest.RobotCentric();
  ...
  private void configureBindings() {
    ...
    joystick.leftTrigger(.5).debounce(.1)
      .whileTrue(
          shooter.setShooterRPS(()->60).andThen(Commands.waitUntil(shooter::getShooterAtSpeed))
          .andThen(indexer.setTopBottomIndexer(-.5, -.5).onlyIf(topLimitSwitchTrigger).until(()->!indexer.getTopLimitSwitch())).alongWith(Commands.print("lt on"))
          .withName("LeftTriggerCommand")
          ).whileFalse(
            Commands.parallel(shooter.cCoastShooter(),indexer.cSetNone(),new PrintCommand("lt off"))
            
          );
    
    // joystick.leftTrigger(.5).debounce(.1)
    // .whileTrue((new PrintCommand("lt")).repeatedly());
    //PLAY NEXT SONG
